...
bool Member::should_convert()
{
    double rebate = this->calculate_rebate();
    if (this->type == "Basic" && rebate >= 75)
        return true;
    if (this->type == "Preferred" && rebate < 75)
        return true;
    return false;
}

double Member::calculate_rebate()
{
    double total = 0;
